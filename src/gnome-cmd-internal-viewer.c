...
static void gc_iv_set_display_mode(GnomeCmdInternalViewer *iv, DISPLAYMODE newmode)
{
	...
	case DISP_IMAGE:
		...
		gtk_statusbar_push(GTK_STATUSBAR(iv->priv->statusbar), iv->priv->sb_msg_ctx_id, _("\t\t\t\tDisplay: Image")) ;
...
CHARSET gc_iv_charsets[] = {
 { N_("Terminal (CP437)"),      "CP437", 'Q'},
 { N_("English (US-ASCII)"), 	"ASCII", 'A'},
 { N_("Arabic (ISO-8859-6)"),	"ISO-8859-6" },
 { N_("Arabic (Windows, CP1256)"), 	"ARABIC" },
 { N_("Arabic (Dos, CP864)"),	"CP864" },
 { N_("Baltic (ISO-8859-4)"),	"ISO-8859-4" },
 { N_("Central European (ISO-8859-2)"),"ISO-8859-2" },
 { N_("Central European (CP1250)"),	"CP1250" },
 { N_("Cyrillic (ISO-8859-5)"),	"ISO-8859-5" },
 { N_("Cyrillic (CP1251)"),     "CP1251" },
 { N_("Greek (ISO-8859-7)"),	"ISO-8859-7" },
 { N_("Greek (CP1253)"),	"CP1253" },
 { N_("Hebrew (Windows, CP1255)"), 	"HEBREW" },
 { N_("Hebrew (Dos, CP862)"),	"CP862" },
 { N_("Hebrew (ISO-8859-8)"),	"ISO-8859-8" },
...
static GtkWidget* internal_viewer_create_menu(GnomeCmdInternalViewer *iv)
{
	...
	menuitem = gc_iv_create_radio_menu_item( &input_mode,_("_ASCII (w/ encodings)"), submenu,
	...
	menuitem = gc_iv_create_radio_menu_item( &input_mode,_("_UTF-8"), submenu,
	...
	/* Character set menu */
	submenu2 = gc_iv_create_sub_menu(_("Character Encodings"), submenu);
	charset = gc_iv_charsets ;
	...
	gc_iv_create_menu_seperator(submenu) ;
	menuitem = gc_iv_create_radio_menu_item( &mode_radioitems_list,_("Text - _Fixed Width"), submenu,
				iv->priv->accel_group,GDK_1, 0,
				G_CALLBACK(gc_iv_menu_view_mode_ascii), iv ) ;
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitem),TRUE);
	menuitem = gc_iv_create_radio_menu_item( &mode_radioitems_list,_("Text - _Variable Width"), submenu,
				iv->priv->accel_group,GDK_2, 0,
				G_CALLBACK(gc_iv_menu_view_mode_pango), iv ) ;
	menuitem = gc_iv_create_radio_menu_item( &mode_radioitems_list,_("_Binary"), submenu,
				iv->priv->accel_group,GDK_3, 0,
				G_CALLBACK(gc_iv_menu_view_mode_binary), iv ) ;
	menuitem = gc_iv_create_radio_menu_item( &mode_radioitems_list,_("_Hex"), submenu,
				iv->priv->accel_group,GDK_4, 0,
				G_CALLBACK(gc_iv_menu_view_mode_hex), iv ) ;
	menuitem = gc_iv_create_radio_menu_item( &mode_radioitems_list,_("_Image"), submenu,
	...
	submenu2 = gc_iv_create_sub_menu(_("_Binary Mode"), submenu) ;
	menuitem = gc_iv_create_radio_menu_item( &binmode_width,_("_20 chars/line"), submenu2,
	...
	menuitem = gc_iv_create_radio_menu_item( &binmode_width,_("_40 chars/line"), submenu2,
	...
	menuitem = gc_iv_create_radio_menu_item( &binmode_width,_("_80 chars/line"), submenu2,
...
gc_iv_file_init_growing_view (GnomeCmdInternalViewer *iv, const char *name, const char *filename)
{
	...
	if ((iv->priv->file = mc_open (filename, O_RDONLY)) == -1)
		return _("init_growing_view: cannot open file");
...
static void gc_iv_refresh_display(GnomeCmdInternalViewer *iv)
{
	...
	{
	case DISP_FIXED: mode_name = _("Text-Fixed");break ;
	case DISP_BINARY: mode_name = _("Binary");break ;
	case DISP_HEX: mode_name = _("Hex");break ;
	case DISP_VARIABLE: mode_name = _("Text-Variable");break ;
	case DISP_IMAGE: mode_name = _("Image");break;
	...
	snprintf(iv->priv->statusbar_msg,sizeof(iv->priv->statusbar_msg),
		_("%d%%    %s    Display: %s    %s wrap"),
		(int)((100*iv->priv->logical_display_offset)/size),
		position,
		mode_name,
		iv->priv->state.ascii_wrap?"":_("no ") ) ;

...
use gtk::gio::{self, ffi::GFileInfo, glib::translate::from_glib_none};
...
#[no_mangle]
pub extern "C" fn ls_colors_get_r(file_info_ptr: *mut GFileInfo, fg: *mut i32, bg: *mut i32) {
    let file_info: gio::FileInfo = unsafe { from_glib_none(file_info_ptr) };
    unsafe {
        *fg = -1;
        *bg = -1;
    }
    if let Some(colors) = ls_colors_get(&file_info) {
        unsafe {
            *fg = colors.fg.map_or(-1, |c| c as i32);
            *bg = colors.bg.map_or(-1, |c| c as i32);
        }
    }
}

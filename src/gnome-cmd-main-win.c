...
destroy (GtkObject *object)
{
	...
	con_home = gnome_cmd_con_list_get_home (
		gnome_cmd_data_get_con_list ());
	
	dir = gnome_cmd_file_selector_get_directory (
		gnome_cmd_main_win_get_left_fs (main_win));
	if (con_home == gnome_cmd_dir_get_connection (dir))	
		gnome_cmd_data_set_start_dir (
			0, gnome_cmd_file_get_path (GNOME_CMD_FILE (dir)));
		
	dir = gnome_cmd_file_selector_get_directory (
		gnome_cmd_main_win_get_right_fs (main_win));
	if (con_home == gnome_cmd_dir_get_connection (dir))
		gnome_cmd_data_set_start_dir (
...
init (GnomeCmdMainWin *mw)
{
	...
	gtk_paned_pack1 (GTK_PANED (mw->priv->paned), mw->priv->file_selector[LEFT],
					 TRUE, TRUE);
	...
	gtk_paned_pack2 (GTK_PANED (mw->priv->paned), mw->priv->file_selector[RIGHT],
					 TRUE, TRUE);

	/*
	if (start_dir_left) 
		gnome_cmd_file_selector_goto_directory (
			gnome_cmd_main_win_get_left_fs (mw), start_dir_left);
	else
		gnome_cmd_file_selector_goto_directory (
			gnome_cmd_main_win_get_left_fs (mw), 
			gnome_cmd_data_get_start_dir (0));
	
	if (start_dir_right) 
		gnome_cmd_file_selector_goto_directory (
			gnome_cmd_main_win_get_right_fs (mw), start_dir_right);
	else
		gnome_cmd_file_selector_goto_directory (
			gnome_cmd_main_win_get_right_fs (mw), 
			gnome_cmd_data_get_start_dir (1));
	*/

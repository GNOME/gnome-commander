...
pub async fn prepare_copy_dialog_show(
    ...
    // if let Some(toggle) =
    //     [query, rename, skip, silent].get(gnome_cmd_data.options.confirm_copy_overwrite)
    // {
...
#[no_mangle]
pub extern "C" fn gnome_cmd_prepare_copy_dialog_show(
    ...
    glib::spawn_future_local(async move {
        prepare_copy_dialog_show(&main_win, &from, &to).await;
    });
}

...
use super::common::{create_category, OptionsWidget};
use crate::data::{FiltersOptions, FiltersOptionsRead, FiltersOptionsWrite};
...
pub fn create_filters_widget() -> Filters {
    let filters = Filters {
...
impl OptionsWidget for Filters {
...
#[no_mangle]
pub extern "C" fn gnome_cmd_create_filters_widget() -> *mut GtkWidget {
    let filters = create_filters_widget();
    filters.load();
    let widget = filters.widget();
    unsafe {
        widget.set_data("filters", filters);
    }
    widget.to_glib_full()
}

#[no_mangle]
pub extern "C" fn gnome_cmd_filters_widget_save(widget: *mut GtkWidget) {
    let widget: Borrowed<gtk::Widget> = unsafe { from_glib_borrow(widget) };
    let filters = unsafe { widget.data::<Filters>("filters") };
    let result = unsafe { filters.unwrap().as_ref() }.save();
    if let Err(error) = result {
        eprintln!("Failed to save filter options: {}", error)
    }
}

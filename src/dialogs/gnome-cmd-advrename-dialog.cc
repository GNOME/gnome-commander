...
inline GnomeCmdAdvrenameDialog::Private::~Private()
{
    gnome_cmd_advrename_template_free(rename_template);
}
...
{
    gnome_cmd_advrename_template_free (dialog->priv->rename_template);
    dialog->priv->rename_template = gnome_cmd_advrename_template_new (component->get_template_entry());
...
void GnomeCmdAdvrenameDialog::Private::on_files_view_popup_menu__update_files (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    gnome_cmd_advrename_template_free (dialog->priv->rename_template);
    dialog->priv->rename_template = gnome_cmd_advrename_template_new (dialog->priv->profile_component->get_template_entry());
...
void GnomeCmdAdvrenameDialog::Private::on_files_view_button_pressed (GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data)
{
    auto dialog = static_cast<GnomeCmdAdvrenameDialog*>(user_data);
...
void GnomeCmdAdvrenameDialog::Private::on_dialog_size_allocate (GObject *dialog, GParamSpec *pspec, gpointer user_data)
{
    int width, height;
    gtk_window_get_default_size (GTK_WINDOW (dialog), &width, &height);

    GNOME_CMD_ADVRENAME_DIALOG (dialog)->defaults.width  = width;
    GNOME_CMD_ADVRENAME_DIALOG (dialog)->defaults.height = height;
}
...
{
    gulong count = gtk_tree_model_iter_n_children (files, NULL);
    ...
    : defaults(cfg)
{
    ...
    priv->rename_template = gnome_cmd_advrename_template_new (priv->profile_component->get_template_entry());
}
